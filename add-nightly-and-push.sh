...
#------------------------------------------------------------------------------
# Adds a nightly binary to a local checkout of hypc-bin repository, updates
...
echo "Adding bin/hypjson-${nightly_version}.js"
[[ ! -e "bin/hypjson-${nightly_version}.js" ]] || die "File bin/hypjson-${nightly_version}.js already exists."
cp "$nightly_binary" "bin/hypjson-${nightly_version}.js"

# Stage the new nightly before running the list update to be able to detect any unintended changes caused by the script.
git add --verbose "bin/hypjson-$nightly_version.js"
...
git add --verbose bin/hypjson-nightly.js
