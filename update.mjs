...
function generateLegacyListJS (builds, releases) {
  return `
var hypjsonSources = ${JSON.stringify(builds, null, 2)};
var hypjsonReleases = ${JSON.stringify(releases, null, 2)};

if (typeof(module) !== 'undefined')
  module.exports = {
    'allVersions': hypjsonSources,
    'releases': hypjsonReleases
  };
`
}
...
function processDir (dir, options, listCallback) {
  readdir(join(__dirname, dir), { withFileTypes: true }, async function (err, files) {
    ...
    const binaryPrefix = (dir === '/bin' || dir === '/wasm' ? 'hypjson' : 'hypc-' + dir.slice(1))
    ...
    processDir(dir, options, function (parsedList) {
      ...
      parsedList.forEach(function (release) {
        if (release.prerelease === undefined) {
          ...
          if (semver.gt(release.version, '0.6.1')) {
            ...
            updateSymlinkSync(
              join('/emscripten-asmjs', 'hypc-emscripten-asmjs-v' + release.longVersion + '.js'),
              join('..', 'bin', release.path)
            )
          }
        }
      ...
      processDir('/wasm', options, function (parsedList) {
        ...
        parsedList.forEach(function (release) {
          if (release.prerelease === undefined) {
            updateSymlinkSync(
              join('/emscripten-wasm32', 'hypc-emscripten-wasm32-v' + release.longVersion + '.js'),
              join('..', 'wasm', release.path)
            )
          }
        })
